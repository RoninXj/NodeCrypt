...
// Decompress volumes to Blob
// 将分卷解压为 Blob
export async function decompressToBlob(volumes, originalHash = null) {
...
async function decompressVolumesToFile(volumes, fileName, originalHash = null) {
	try {
		const blob = await decompressToBlob(volumes, originalHash);
...
export function setupFileSend({
	...
	if (attachBtn) {
		...
		attachBtn.addEventListener('click', (e) => {
			...
			// 修改文件输入框的 accept 属性以支持图片和视频
			const fileInput = document.querySelector(fileInputSelector);
			if (fileInput) {
				fileInput.setAttribute('accept', 'image/*,video/*,application/*,text/*');
			}
...
function handleFileStart(message, isPrivate) {
	...
	// 根据文件名识别媒体类型
	let msgType = isPrivate ? 'file_private' : 'file';
	const extension = fileName.split('.').pop().toLowerCase();
	...
	if (!isArchive) {
		if (imageExtensions.includes(extension)) {
			// 暂时保留 file 类型，但在渲染层做处理，或者直接切换类型
			// 为了保持向后兼容和分卷逻辑，我们保持 file 结构，但在聊天气泡中渲染媒体
		}
	}
