...
export function addMsg(text, isHistory = false, msgType = 'text', timestamp = null) {
	...
	} else if (msgType === 'video' || msgType === 'video_private') {
		// Handle video messages
		const safeVideoSrc = typeof text === 'string' ? escapeHTML(text).replace(/javascript:/gi, '') : '';
...
export function addOtherMsg(msg, userName = '', avatar = '', isHistory = false, msgType = 'text', timestamp = null) {
	...
	const bubbleWrap = createElement('div', {
		...
		// Handle video messages
		const safeVideoSrc = typeof msg === 'string' ? escapeHTML(msg).replace(/javascript:/gi, '') : '';
		contentHtml = `<video src="${safeVideoSrc}" controls class="bubble-video"></video>`;
	} else if (msgType === 'file' || msgType === 'file_private') {
		// Handle file messages
...
function renderFileMessage(fileData, isSender) {
	...
	// Check if this is an image or video that we have already received locally
	if (!isArchive && transfer && transfer.status === 'completed' && transfer.volumeData) {
	...
	return `
		<div class="file-message" data-file-id="${fileId}">
			...
			<div class="file-preview-area" style="display: none;"></div>
